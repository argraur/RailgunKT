...
class Jenkins(url: String, user: String, pass: String) {
    ...
    fun build(jobName: String, message: Message) {
        ...
        message.channel.sendMessage(
            ...
            val embed = Embed(
                when {
                    (build.result == BuildResult.SUCCESS) -> { status = "Successful"; Color.GREEN }
                    (build.result == BuildResult.FAILURE) -> { status = "Failed"; Color.RED }
                    (build.result == BuildResult.ABORTED) -> { status = "Aborted"; Color.GRAY }
                    else -> { status = "Unknown"; Color.ORANGE }
                }
            )
